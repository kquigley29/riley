...
void RileyNet::print_architecture() {std::cout << "Architecture: " << neural_network.get_architecture() << "\n";}


void RileyNet::educate(const OpenNN::Matrix<double> &matrix_data) {

    // Set the matrix as the data set
    // Assign uses to the columns
    OpenNN::DataSet data_set(matrix_data);
    data_set.set_input();
    for (int i = 0; i < 37; i++) {data_set.set_column_use(i, OpenNN::DataSet::VariableUse::Target);}
